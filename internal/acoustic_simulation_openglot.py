...
def calc_p_boundary(pj,boundaries,pguide_l,pguide_m,pguide_n,Y_l,Y_m,Y_n,l,m,n,ghost_point_indices,edge_admittance,glottis_admittance):
    ...
    # We can use a different value for the admittance at different bouundaries
    # Here, the admittance at the glottis is different
    if n == 1 and boundaries[l,m,n] == 0:
        admittance = glottis_admittance
    else:
        admittance = edge_admittance
...
def calc_p_SRL(pj,boundaries,pguide_l,pguide_m,pguide_n,Y_l,Y_m,Y_n,edge_admittance,glottis_admittance):
        ...
        for m in range(1,pj.shape[2]-1):
            for n in range(1,pj.shape[3]-1):
                ...
                if len(ghost_point_indices) > 0:
                    ...
                    pj,pguide_l,pguide_m,pguide_n = calc_p_boundary(pj,boundaries,pguide_l,pguide_m,pguide_n,Y_l,Y_m,Y_n,l,m,n,ghost_point_indices,edge_admittance,glottis_admittance)
                else:
...
def snapshot3d(i, pj, path, start_time, loop_start, sim_steps):
...
def check_sim_done(sim_output_path, nodes, sim_steps):
    if os.path.isfile(sim_output_path+'\\snapshot'+str(sim_steps-1)+'.npy'):
        ...
        if os.path.isfile(sim_output_path+'\\nodevalues'+str(node_positions[0])+'.npy'):
            print('VTTFs already generated.')
            return True

def DWM3D(boundaries, Yvals, nodes, sim_name, sim_output_path, edge_admittance = 1.0, glottis_admittance = 1.0, use_wang_impedance = False):
    deltaX = 1E-3
    ...
    if check_sim_done(sim_output_path, nodes, sim_steps):
        return

    source_node = tuple(nodes[0])
    ...
    conditions_file = open(sim_output_path+'\\'+str(sim_name)+'.txt', 'w')
    conditions_file.write(sim_conditions)
    conditions_file.close()

    for t in range(sim_steps):
        loop_start = time.time()
        pj,pguide_l,pguide_m,pguide_n = calc_p_SRL(pj,boundaries,pguide_l,pguide_m,pguide_n,Y_l,Y_m,Y_n,edge_admittance,glottis_admittance)
        ...
        if t % snapshot_every_x_frames == 0:
            snapshot3d(t, pj[1], sim_output_path, start_time, loop_start, sim_steps)
